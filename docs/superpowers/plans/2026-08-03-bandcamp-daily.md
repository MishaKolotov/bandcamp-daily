...
Накопились в ревью и обязательны для оставшихся задач — план в основной части местами их ещё не отражает.

1. **Состояние «уже показывали» ключуется по URL, а не по `itemId`.** У релизов, найденных через подписки, числового id на странице группы нет, и `fresh.ts` подставляет туда детерминированный хеш URL. Тот же релиз, всплывший позже через Discover, придёт с настоящим `item_id` — по `itemId` это два разных релиза. URL — единственный стабильный ключ. `itemId` остаётся только компактной ручкой для `callback_data` в Telegram. Касается задач 14, 18, 19, 21.
2. **Обход подписок делается один раз за прогон, а не по разу на бакет.** Страницы `/music` намеренно не кэшируются, подписок 166, и наивный вызов `freshCandidates` для каждого из трёх бакетов даёт 498 запросов вместо 166. Задача 21 обязана собрать релизы подписок один раз и раздать их бакетам.
3. **`deriveStopTags` требует `seedTags` своего бакета и `releasesSampled`.** Оба поля обязательные, дефолтов нет: без первого бакет заносит собственный жанр в свой же стоп-лист, без второго порог частоты не интерпретируем. Черновик вызова в задаче 15 устарел — копировать его дословно нельзя.
4. **Скрипт сборки профиля печатает число тегов и релизов по каждому бакету.** Хардкор-бакет засеян намеренно узким списком, и слишком тонкий профиль должен быть виден глазами до того, как канал начнёт постить, — а не обнаруживаться через месяц по качеству подборок.
5. **Чужие коллекции читаются с ограничением примерно в размер коллекции владельца.** Знаменатель близости — минимум из двух размеров, и для коллекции больше 223 он уже упёрся в размер владельца, так что вычитывать тысячи позиций у всеядного коллекционера бессмысленно: это только время. Задача 20 передаёт `maxPages` явно, а не наследует лимит, подобранный под владельца. Иначе худший случай недельного прогона — около 4800 запросов, больше часа.
6. **Архивный пул собирается один раз за прогон и с запасом, а не по разу на бакет.** Голоса соседей не разделены по жанрам, поэтому если верхушку лидерборда займёт один жанр, остальным двум бакетам архивных кандидатов не достанется вовсе — и это будет происходить каждый день, а не изредка. Задача 21 берёт пул заметно шире тридцати позиций и фильтрует его по бакетам уже скорером.
7. **Профиль отдаёт seed-тегу фиксированный вес 0.5, остальным — нормировку к максимуму 1.** Скорер откалиброван ровно под это: `MATCH_FLOOR = 0.5` (релиз с одним общим тегом проходит), `STRONG_MATCH = 1.5` (потолок «seed + самый характерный тег»). Менять одно без другого нельзя.
8. **2026-08-04: подборщик отдаёт только новинки.** Владелец решил, что архивная половина не нужна — граф соседей по вкусу (`bin/neighbors.ts`, `data/neighbors.json`), еженедельный пересчёт (`neighbors-weekly.yml`) и архивный пул кандидатов удалены из кода целиком, а не выключены флагом. Пункты 5 и 6 выше остаются как история решений, принятых для уже удалённой половины, а не как описание текущего поведения.
