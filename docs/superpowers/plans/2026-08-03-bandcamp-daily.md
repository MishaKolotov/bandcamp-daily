...
5. **Профиль отдаёт seed-тегу фиксированный вес 0.5, остальным — нормировку к максимуму 1.** Скорер откалиброван ровно под это: `MATCH_FLOOR = 0.5` (релиз с одним общим тегом проходит), `STRONG_MATCH = 1.5` (потолок «seed + самый характерный тег»). Менять одно без другого нельзя.
