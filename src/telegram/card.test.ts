...
test('карточка владельца несёт ровно две кнопки — скип и другой', () => {
  const card = buildCard(candidate, 'crust', ['crust']);
...
/**
 * Реальный релиз с шумной смесью тегов, как они приходят с Bandcamp —
 * вперемешку город ('olympia'), настоящие жанры ('punk', 'crust',
 * 'hardcore') и явный мусор ('energy', 'friendship', 'greatest label of all
 * time', 'iron lung' — имя самого лейбла как тег). Раньше фикстура
 * использовалась и для проверки хэштег-фильтра в посте канала — той фичи
 * больше нет, но карточка владельца по-прежнему должна показывать эти теги
 * все как есть, без фильтрации.
 */
