...
export class TelegramApiError extends Error {
   ...
   * повторить позже. 400/401/403/404 повтором не лечатся — плохой токен, чат не найден,
   * владелец не нажимал /start и т.п. — там нужно сообщать владельцу, а не долбить снова.
   */
...
export class Telegram {
   ...
   *
   * Джоб ждёт нажатий кнопок отведённое окно (сейчас 20 минут, LISTEN_MINUTES)
   * независимо от стратегии опроса — раннер GitHub Actions платит за это время стены в
   * любом случае, живым ли HTTP-соединением внутри getUpdates или собственным sleep()
   * между короткими запросами. Разница не в минутах CI, а в количестве запросов и риске
   * упереться в рейт-лимит Telegram: с timeout=0 (busy-poll) без ручной паузы между
   * вызовами набегают тысячи запросов; с 30-секундным long-poll — по два в минуту.
   * Долгий опрос не удлиняет и не укорачивает сам джоб — он просто ждёт эффективнее.
   */
  ...
  /**
   * Сносит сообщение целиком. Общий лимит Telegram «удалять можно только то,
   * что моложе 48 часов» на наш случай не распространяется: сообщение старше
   * 48 часов бот всё равно может удалить, если отправил его сам — а в личке
   * владельца бот удаляет исключительно свои же карточки.
   *
   * Возвращает `true` при успехе; отсутствующее сообщение (владелец снёс
   * карточку руками) Telegram отдаёт ошибкой, а не `false` — зовущий код
   * обязан решить сам, считать это фатальным или нет. И `handleSkip`/
   * `handleNext` в `./approve.ts`, и подчистка хвостов в
   * `../pipeline/daily.ts` зовут это best-effort: состояние к тому моменту
   * уже согласовано, а неудавшееся удаление косметическое.
   */
  deleteMessage(payload: { chat_id: string | number; message_id: number }): Promise<boolean> {
