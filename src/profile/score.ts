 ...
 * бакет отбраковывает всё подряд без шанса на восстановление — при паре
 * скипов в неделю это месяц-два до тихой смерти бакета. Потолок в 1
 ...
 * четырёх бакетов разом, а не вкусовая настройка одного из них. Дублировать
...
export function score(
  ...
  // приходят из Discover и дискографий подписок — fresh.ts проставляет туда
  // 0, потому что взять число неоткуда: ни ответ Discover, ни ld+json
  // страницы релиза его не содержат. Раньше здесь стоял логарифмический член
  // от этого поля — он всегда давал ровно ноль, то есть был мёртвой веткой
  // скоринга с комментарием, описывающим поведение, которого нет. Если
  // популярность когда-нибудь понадобится, сначала нужно найти, откуда её
  // брать, и только потом возвращать член.
