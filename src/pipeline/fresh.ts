...
export interface FreshOptions {
   ...
   * не написан) прогонит через `freshCandidates` все три бакета с одним и
   * тем же полным списком подписок, каждая из 166 страниц /music будет
   * скачана трижды за прогон вместо одного раза. Эта функция такого не
   * решает и не должна: у неё нет памяти между вызовами. Развести общий
   * список подписок по бакетам, свести fetch дискографий в один проход и
   * раздать результат трём бакетам — забота вызывающего кода.
   */
  subdomains: string[];
  now: Date;
  maxAgeDays: number;
