 ...
 *   15 страниц — это до 1500 позиций (по 100 на страницу; первый живой
 *   прогон показал несколько соседей, упёршихся ровно в 1500). С запасом
 *   покрывает 223 релиза владельца, дальше которых дробь всё равно не
 *   меняется. Без явного предела (дефолт в fetchFanItems — MAX_PAGES=60,
 ...
 *   Это потолок: у большинства кандидатов коллекция кончается задолго до
 *   лимита (more_available становится false), и обход останавливается сам —
 *   реальное время заметно меньше потолка.
 */
